    ...
    return (
        <nav className="fixed top-0 left-0 w-full z-50 bg-white shadow-md">
            <div className="container flex items-center justify-between px-6">
                                ...
                                {menu.dropdown && (
                                    <ul className="absolute left-1/2 transform -translate-x-1/2 mt-2 hidden group-hover:block bg-white shadow-lg rounded-lg p-2 w-40">
                                        {menu.dropdown.map((item) => (
                                            <li key={item.id} className="px-4 py-2 hover:bg-gray-100 border-b last:border-b-0 border-gray-200">
                                                <Link to={item.path} className="block text-gray-800 hover:text-sky-300 transition duration-100">
                                                    {item.title}
                                                </Link>
                                            </li>
                                        ))}
                ...
                {/* Register Button */}
                <button className="font-bold hidden lg:flex lg:mr-5 md:flex items-center bg-blue-300 text-white px-4 py-2 rounded-lg cursor-pointer hover:bg-sky-300 shadow-md hover:shadow-sky-300 transition duration-300">
            ...
            {/* Mobile Menu */}
            <div
                className={`lg:hidden fixed top-20 right-0 rounded-lg h-full w-56 bg-white shadow-lg p-5 transform ${isMenuOpen ? "translate-x-0" : "translate-x-full"
