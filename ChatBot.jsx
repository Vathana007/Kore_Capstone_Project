...
const ChatBotUI = () => {
  ...
  // Instead of initializing from a generic key, start with an empty array
  ...
  // Load user-specific conversation history when the user changes
  useEffect(() => {
    if (user && user._id) {
      const stored = localStorage.getItem(`chatHistory_${user._id}`);
      setAllConversations(stored ? JSON.parse(stored) : []);
    }
  }, [user]);

  // Save conversations to localStorage under a key that includes the user ID
  useEffect(() => {
    if (user && user._id) {
      localStorage.setItem(`chatHistory_${user._id}`, JSON.stringify(allConversations));
    }
